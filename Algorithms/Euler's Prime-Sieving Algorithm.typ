...
Define "visit" as marking a number with `isprime` using the inner loop. It is clear that the overall time complexity is the same as the number of times the inner loop is iterated; that is, the number of "visits" that occur in total.

#underline[Claim]: The number of visits made to each integer from $2$ to $n$ is at most $1.$

#underline[Proof]:

  If the number $i$ is itself a prime, then it can only be visited by the outer loop, and cannot be visited by the inner loop. This is because to be visited by the inner loop, the number has to be decomposed into the product of two positive integers, namely `i` and `primes[j]`, which according to the code are both greater than $1$. Hence, if a number is prime in the range $[1, n]$, it is not visited by the inner loop. That is, the number of visits to the primes is $0$.
