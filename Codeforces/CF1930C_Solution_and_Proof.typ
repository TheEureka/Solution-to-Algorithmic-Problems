...
#proof[
  ...
  Notice that all _good_ ararys of length $n + 1$ are formed by appending a value $x$ with $0 <= x <= n$ at the end of a _good_ array of length $n$.
...
#proof[
  ...
  Moroever, when such a collision scenario occurs, the answer array that is decreased in length apparently doesn't grow lexicographically larger. Because collisions are equivalent to removing some answers from the optimal answer array,
